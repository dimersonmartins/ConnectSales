...
namespace DataServer
{
    ...
    {
        public Dictionary<string, string> Cidades()
        {
            Dictionary<string, string> cidades = new Dictionary<string, string>
            {
                ...
                { "IMPERATRIZ","23051~NETSALES_BASE_SOC" },
        ...
        public Dictionary<string, string> Estatos()
        {
            Dictionary<string, string> estatos = new Dictionary<string, string>
            {
                {"AC","AC"},
                {"AL","AL"},
                {"AM","AM"},
        ...
        public Dictionary<int, string> statusNetsales()
        {
            return new Dictionary<int, string>()
            {
                { 1, "CANCELADO" },
                { 2, "CONECTADO" },
                { 3, "CONECTADO (DESABILITACAO)" },
                { 4, "CONECTADO (DESC. OPCAO DESPACHADA)" },
                { 5, "CONECTADO (DESCONEXAO - OPCAO)" },
                { 6, "CONECTADO (DESCONEXAO PARCIAL)" },
                { 7, "CONECTADO (DESCONEXAO SSI)" },
                { 8, "CONECTADO (DESCONEXAO TOTAL)" },
                { 9, "CONECTADO (DESCONEXAO TOTAL DESPACHADA)" },
                { 10, "CONECTADO (MUDANCA DE ENDERECO)" },
                { 11, "CONECTADO(DESABILITADO PARCIAL)" },
                { 12, "CONECTADO(DESCONEXAO SSI EBT)" },
                { 13, "DESAB PARCIAL(DESCONEXAO SSI EBT)" },
                { 14, "DESABIL PARCIAL(DESABILITADO TOTAL)" },
                { 15, "DESABIL PARCIAL(DESCONECTADO P/ INADIM)" },
                { 16, "DESABIL PARCIAL(DESCONECTADO-OPCAO)" },
                { 18, "DESABIL TOTAL(DESABILITADO PARCIAL)" },
                { 19, "DESABILITADO" },
                { 20, "DESABILITADO  BLOQUEIO PARCIAL" },
                { 21, "DESABILITADO  BLOQUEIO TOTAL" },
                { 22, "DESABILITADO BP (DESABILITADO BT)" },
                { 23, "DESABILITADO BP (DESCONECTADO P/ INADIM)" },
                { 24, "DESABILITADO BT (DESCONECTADO P/ INADIM)" },
                { 25, "DESABILITADO (CONECTADO)" },
                { 26, "DESABILITADO (DESCONEXAO PARCIAL)" },
                { 27, "DESABILITADO (DESCONEXAO SSI)" },
                { 28, "DESABILITADO PARCIAL" },
                { 29, "DESABILITADO PARCIAL (DESCONEXAO SSI)" },
                { 30, "DESABILITADO PARCIAL(CONECTADO)" },
                { 31, "DESABILITADO(DESCONEXAO SSI EBT)" },
                { 32, "DESC INAD TOTAL (DESCONEXAO SSI)" },
                { 33, "DESC INAD(TOTAL)(DESCONEXAO SSI EBT)" },
                { 34, "DESC PARCIAL(DESCONEXAO SSI EBT)" },
                { 35, "DESCONECTADO - INADIMPLENCIA (TOTAL)" },
                { 36, "DESCONECTADO - OPCAO" },
                { 37, "DESCONECTADO - OPCAO (MUD. ENDERECO)" },
                { 38, "DESCONECTADO DIVERGENCIA CAD SSI EBT" },
                { 39, "DESCONECTADO DIVERGENCIA DE CADASTRO SSI" },
                { 40, "DESCONECTADO PARCIAL" },
                { 41, "DESCONECTADO PARCIAL (DESCONEXAO SSI)" },
                { 42, "DESCONECTADO PARCIAL (MUDANCA ENDERECO)" },
                { 43, "DESCONECTADO PARCIAL (RECONEXAO)" },
                { 44, "DESCONECTADO PARC.(RECONEXAO DESPACHADA)" },
                { 45, "DESCONECTADO PARC.(S/DIVIDA S/RECONEXAO)" },
                { 46, "DESCONECTADO POR INADIMPLENCIA" },
                { 47, "DESCONECTADO (REINSTALACAO)" },
                { 48, "MUD. ENDERECO (DESCONEXAO - OPCAO)" },
                { 49, "MUD. ENDERECO (SEM INST NOVO ENDERECO)" },
                { 50, "MUDANCA DE ENDERECO" },
                { 51, "NAO POSSUI" },
                { 52, "PEND. EM INSTALACAO" },
                { 53, "PEND. EM LIBERACAO" }
            };
         }
    }
}
